...
interface Props {
  ...
  onSave: (data: EntryType, action: 'close' | 'clear') => void;
  onCancel: () => void;
}
...
export function EntryForm({ initialData, onSave, onCancel }: Props) {
  const { register, handleSubmit, reset, formState: { errors } } = useForm<EntryType>({
  ...
  const onSubmit = (data: EntryType, e?: React.BaseSyntheticEvent) => {
    // Get action from the button that triggered the submit
    const nativeEvent = e?.nativeEvent as SubmitEvent;
    const submitter = nativeEvent?.submitter as HTMLButtonElement;
    const action = (submitter?.dataset.action as 'close' | 'clear') || 'close';
      ...
      <div className="sticky top-0 z-20 bg-[#F9FAFB] flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center py-[16px] mb-[24px]">
        ...
        <div className="flex flex-wrap gap-3 w-full sm:w-auto">
          <button 
            type="button" 
            onClick={onCancel}
            className="flex-1 sm:flex-none flex items-center justify-center gap-[8px] bg-white border border-[#D1D5DB] text-[#374151] px-[16px] py-[10px] rounded-[6px] text-[13px] font-medium transition-colors cursor-pointer hover:bg-gray-50 shadow-sm"
          >
            <X size={16} /> Đóng
          </button>
          ...
          <button 
            type="submit"
            data-action="clear"
            className="flex-1 sm:flex-none flex items-center justify-center gap-[8px] bg-white border border-[#D1D5DB] text-red-600 px-[16px] py-[10px] rounded-[6px] text-[13px] font-medium transition-colors cursor-pointer hover:bg-red-50 hover:border-red-200 shadow-sm"
          >
            <Eraser size={16} /> Lưu & Nhập mới
          </button>

          <button 
            type="submit"
            data-action="close"
            className="flex-1 sm:flex-none flex items-center justify-center gap-[8px] bg-[#2563EB] text-white px-[20px] py-[10px] rounded-[6px] text-[13px] font-medium transition-colors cursor-pointer hover:bg-blue-700 shadow-sm"
          >
