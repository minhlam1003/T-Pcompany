...
export default function App() {
  ...
  const [allUsers, setAllUsers] = useState<UserProfile[]>(() => {
    ...
    return saved ? JSON.parse(saved) : [
      { uid: 'system-admin', email: 'admin@system.local', username: 'Administrator', role: 'admin', status: 'active' }
    ];
  });
  ...
  const stats = useMemo(() => {
    const completed = data.filter(item => item.maHoSoLuuTru && item.hoTenChuSuDung && item.soGiayChungNhan).length;
    const missingGCN = data.filter(item => !item.soGiayChungNhan).length;
  ...
  useEffect(() => {
    if (activeTab === 'browser' && searchQuery) {
      document.title = `Tìm thấy ${filteredData.length} / ${data.length} hồ sơ`;
    } else {
  ...
  const handleLogout = () => {
    addNotification('success', 'Hệ thống đang chạy ở chế độ Local độc lập.');
  };
  ...
  const handleUpdateUserStatus = async (uid: string, role: UserProfile['role'], status: UserProfile['status']) => {
    if (!uid) return;
    setAllUsers(prev => prev.map(u => u.uid === uid ? { ...u, role, status } : u));
                       ...
                       {settingsActiveGroup === 99 ? (
                         <div className="flex-1 flex flex-col animate-in fade-in duration-500">
                           ...
                           <div className="flex-1 overflow-y-auto pr-2">
                             ...
                             {/* User Table Header */}
                             <div className="bg-gray-50 p-3 rounded-t-xl border-x border-t border-gray-100 grid grid-cols-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">
                             ...
                             <div className="border border-gray-100 rounded-b-xl overflow-hidden divide-y divide-gray-50">
                               {allUsers.map(user => (
                                 <div key={user.uid} className={cn(
                                   "p-3 grid grid-cols-4 items-center bg-white hover:bg-gray-50/50 transition-colors",
                                   ...
                                   <div>
                                     <div 
                                       onClick={() => handleUpdateUserStatus(user.uid, user.role, user.status === 'active' ? 'suspended' : 'active')}
                                       className={cn(
