...
export default function App() {
  ...
  const filteredData = useMemo(() => {
    ...
    if (sortConfig) {
      result = [...result].sort((a, b) => {
        ...
        // Enhanced numeric extraction and comparison for sorting
        const numericFields = ['nhanDe', 'soToBanDo', 'soThua', 'tongSoTaiLieu', 'soLuongTo', 'soLuongTrang', 'hopSo', 'ke'];
  ...
  const canUserEditRecord = (user: UserProfile | null, record: LandData) => {
    ...
    if (user.role === 'employee') {
      // Allow editing imported data, but specific fields will be protected in the UI
      
      // Must be within assigned nhanDe range
      const rawNhanDe = String(record.nhanDe || '').trim();
      const numericMatches = rawNhanDe.match(/(\d+)/g);
      const nhanDeValue = numericMatches ? parseInt(numericMatches[0]) : NaN;
      
      if (!rawNhanDe) return { can: false, reason: 'Hồ sơ chưa có thông tin Nhan đề' };
      if (isNaN(nhanDeValue)) return { can: false, reason: 'Nhan đề không chứa số để kiểm tra phân quyền' };
      ...
      if (start !== null && !isNaN(start) && nhanDeValue < start) return { can: false, reason: `Ngoài dải nhan đề cho phép (Từ ${start})` };
      if (end !== null && !isNaN(end) && nhanDeValue > end) return { can: false, reason: `Ngoài dải nhan đề cho phép (Đến ${end})` };
      
      return { can: true };
    }
    return { can: false, reason: 'Quyền hạn không xác định' };
  };

  const handleEditEntry = (item: LandData) => {
    const { can, reason } = canUserEditRecord(currentUser, item);
    if (!can) {
      addNotification('error', reason || 'Bạn không có quyền chỉnh sửa hồ sơ này.');
      return;
    }
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-8">
                          {activeFields.map(field => (
                            <div key={field.key} className={cn("space-y-1.5", (field.type === 'textarea' || field.key === 'hoTenChuSuDung') && "md:col-span-2")}>
                              ...
                              {(() => {
                                const isExcelField = ['nhanDe', 'hopSo', 'tieuDeHoSo', 'thoiGianBatDau', 'thoiGianKetThuc', 'soLuongTo'].includes(field.key);
                                const isFieldProtected = currentUser?.role === 'employee' && (
                                  activeGroupIndex === 0 || // Protect Group 1
                                  (currentEntry.isImported && isExcelField) // Protect imported fields in imported records
                                );
                        ...
                        <table className="w-full text-sm text-left border-collapse">
                          <thead>
                            <tr className="bg-gray-50/50">
                              ...
                              <th 
                                className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-gray-400 cursor-pointer hover:text-blue-500 transition-colors"
                                onClick={() => toggleSort('soToBanDo')}
                              >
                                <div className="flex items-center gap-1.5">
                                  Tờ BĐ
                                  {sortConfig?.key === 'soToBanDo' ? (
                                    sortConfig.direction === 'asc' ? <ArrowUp size={12} className="text-blue-500" /> : <ArrowDown size={12} className="text-blue-500" />
                                  ) : (
                                    <ArrowUpDown size={12} className="text-gray-300" />
                                  )}
                              ...
                              <th 
                                className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-gray-400 cursor-pointer hover:text-blue-500 transition-colors text-center"
                                onClick={() => toggleSort('hopSo')}
                              >
                                <div className="flex items-center justify-center gap-1.5">
                                  Hộp/Kệ
                                  {sortConfig?.key === 'hopSo' ? (
                                    sortConfig.direction === 'asc' ? <ArrowUp size={12} className="text-blue-500" /> : <ArrowDown size={12} className="text-blue-500" />
                                  ) : (
                                    <ArrowUpDown size={12} className="text-gray-300" />
                                  )}
                              ...
                              <th 
                                className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-gray-400 cursor-pointer hover:text-blue-500 transition-colors text-center"
                                onClick={() => toggleSort('soGiayChungNhan')}
                              >
                                <div className="flex items-center justify-center gap-1.5">
                                  Số GCN
                                  {sortConfig?.key === 'soGiayChungNhan' ? (
                                    sortConfig.direction === 'asc' ? <ArrowUp size={12} className="text-blue-500" /> : <ArrowDown size={12} className="text-blue-500" />
                                  ) : (
                                    <ArrowUpDown size={12} className="text-gray-300" />
                                  )}
                              ...
                              <th 
                                className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-gray-400 cursor-pointer hover:text-blue-500 transition-colors"
                                onClick={() => toggleSort('isImported')}
                              >
                                <div className="flex items-center gap-1.5">
                                  Nguồn
                                  {sortConfig?.key === 'isImported' ? (
                                    sortConfig.direction === 'asc' ? <ArrowUp size={12} className="text-blue-500" /> : <ArrowDown size={12} className="text-blue-500" />
                                  ) : (
                                    <ArrowUpDown size={12} className="text-gray-300" />
                                  )}
                              ...
                              <th 
                                className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-gray-400 cursor-pointer hover:text-blue-500 transition-colors text-right pr-6"
                                ...
                                >
                                  ...
                                  <td className="px-6 py-4">
                                    <div className="text-[11px] font-bold text-gray-600">
                                      {item.soToBanDo || '?'}/{item.soThua || '?'}
                                    </div>
                                  </td>
                                  <td className="px-6 py-4 text-center">
                                    <div 
                                      className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-gray-50 border border-gray-100 text-[10px] font-bold text-gray-500 hover:border-blue-200 hover:text-blue-600 cursor-pointer transition-all"
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        toggleSort('hopSo');
                                      }}
                                    >
                                      H{item.hopSo || '?'}-K{item.ke || '?'}
                                    </div>
                                  </td>
                                  <td className="px-6 py-4">
                                    ...
                                    <div className="flex items-center justify-center gap-1">
                                      ...
                                      {(() => {
                                        ...
                                        return can ? (
                                          ...
                                          <div className="p-2 text-gray-100 cursor-not-allowed" title={reason}>
                                            <Lock size={14} />
                                          </div>
                                        );
                                      })()}
